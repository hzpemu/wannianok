...
var temp = Animals[(cld[d].lYear-4)%12];  //20190705fix  allstr[0] = Animals[(yyyy-4)%12];    20230518->停用allstr[0] = Animals[(cld[d].lYear-4)%12];
...
allstr[5]=moonxzgeti(cld[d].lDay,tttt,min,allstr[2])+1; //20190705 index+1 //20230518有时会有BUG会变成空 2023-05-16~18 15:59 已于修正20230518moonxzget里
...
var Lunarfix=Lunar.fromYmdHms(cld[d].lYear,cld[d].lMonth,cld[d].lDay, tttt, min, 0);
var oldysx=Lunarfix.getYearShengXiaoByLiChun();var oldygz=Lunarfix.getYearGanByLiChun()+Lunarfix.getYearZhiByLiChun();var oldmgz=Lunarfix.getMonthGan()+Lunarfix.getMonthZhi();var olddgz=Lunarfix.getDayGan()+Lunarfix.getDayZhi();var oldtgz=Lunarfix.getTimeGan()+Lunarfix.getTimeZhi();
var Lunarfixfix=null;
if(tttt==23){
	tttt=0;
	Lunarfix=Lunar.fromYmdHms(cld[d].lYear,cld[d].lMonth,cld[d].lDay, tttt, min, 0);
	Lunarfix=Lunarfix.next(1);
	var newysx=Lunarfix.getYearShengXiaoByLiChun();var newygz=Lunarfix.getYearGanByLiChun()+Lunarfix.getYearZhiByLiChun();var newmgz=Lunarfix.getMonthGan()+Lunarfix.getMonthZhi();var newdgz=Lunarfix.getDayGan()+Lunarfix.getDayZhi();var newtgz=Lunarfix.getTimeGan()+Lunarfix.getTimeZhi();
	oldysx=newysx;oldygz=newygz;oldmgz=newmgz;olddgz=newdgz;oldtgz=newtgz;
	}
//var Lunarfix=Lunar.fromYmdHms(cld[d].lYear,cld[d].lMonth,cld[d].lDay, tttt, min, 0);
//if(!Lunarfixfix){Lunarfix=Lunarfixfix;}
// 往后推两天，即后天
//console.log(d.next(2));
allstr[0] =oldysx;
allstr[1] = a1.indexOf(allstr[0].substr(0,1))+1; //20190705 index+1
allstr[12] = oldygz;//20230518->停用allstr[12] =cld[d].cYear;
allstr[13]=g1.indexOf(allstr[12].substr(0,1))+1; //20190705 index+1
allstr[14]=z1.indexOf(allstr[12].substr(1,1))+1; //20190705 index+1
allstr[15] = oldmgz;//20230518->停用allstr[15] = cld[d].cMonth;
...
allstr[21] =oldtgz;//20230518->停用allstr[21] =rtgan1+z11;
...
//allstr[27]=Lunarfix.getYearGanExact();//JSON.stringify(Solar.fromYmd(yyyy,mmmm,dddd), null, 4);
//var solar =Solar.fromYmd(yyyy,mmmm,dddd);
//allstr[28]=Lunarfix.getYearZhiExact();//solar.toFullString();
//allstr[29]=Lunarfix.getMonthGanExact();//solar.getLunar().toFullString();
//allstr[30]=Lunarfix.getMonthZhiExact();
//allstr[31]=Lunarfix.getYearShengXiaoExact();
/*
你使用了SimpleDateFormat来将字符串转换为Date类型？
...
console.log('获取年的生肖（以正月初一起）：'+d.getYearShengXiao());
console.log('获取月的生肖（以节交接当天起）：'+d.getMonthShengXiao());
console.log('获取日的生肖：'+d.getDayShengXiao());
console.log('获取时辰的生肖：'+d.getTimeShengXiao());

console.log('获取年的生肖（以立春当天起）：'+d.getYearShengXiaoByLiChun());
console.log('获取年的生肖（以立春交接时刻起）：'+d.getYearShengXiaoExact());
console.log('获取月的生肖（以节交接时刻起）：'+d.getMonthShengXiaoExact());
...
//calendarextented.solar2lunar(yyyy,mmmm,dddd);
//allstr[27]=JSON.stringify(calendarextented.default.solar2lunar(yyyy,mmmm,dddd), null, 4);
//JSON.stringify(calendar.default.solar2lunar(1987, 11, 1), null, 4)
//JSON.stringify(calendar.default.solar2lunar(yyyy,mmmm,dddd), null, 4);
return allstr;
}
